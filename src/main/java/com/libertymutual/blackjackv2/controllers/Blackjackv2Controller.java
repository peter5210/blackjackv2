...
import com.libertymutual.blackjackv2.model.AceCard;
...
import com.libertymutual.blackjackv2.model.FaceCard;
...
public class Blackjackv2Controller {
	...
	//
	// if (dealerFirstTotal > playerFirstTotal || dealerFirstTotal >
	// playerSecondTotal) {
	// if (dealerSecondTotal > playerFirstTotal || dealerSecondTotal >
	// playerSecondTotal) {
	...
	// int playerSecondTotal = playerOne.totalCardValue()[1];
	//
	// if ((playerFirstTotal == 21 || playerSecondTotal == 21) && (dealerFirstTotal
	// < 21 && dealerSecondTotal < 21)) {
	// playerBlackjackWin = true;
	// }
	// if ((dealerFirstTotal == 21 || dealerSecondTotal == 21) && (playerFirstTotal
	// == 21 || playerSecondTotal == 21)) {
	// playerPush = true;
	// }
	// if (dealerFirstTotal > playerFirstTotal || dealerFirstTotal >
	// playerSecondTotal) {
	...
	public ModelAndView playerHit() {
		...
		public ModelAndView playerStand() {
			...
			if (dealer.dealerBust()) {
				...
				if (dealer.isPlayerWin() || dealer.isPlayerBlackjackWin()) {
					ModelAndView modelAndView = new ModelAndView("blackjack2/gameover");

					// This is mandatory for the game over form
					modelAndView.addObject("dealersHand", dealer.getHand());
					modelAndView.addObject("playersHand", playerOne.getHand());
					modelAndView.addObject("Busted", "You Won");
			...
			// figure out who won and make the applicable payout.
				...
				modelAndView.addObject("dealersHand", dealer.getHand());
				modelAndView.addObject("playersHand", playerOne.getHand());
				modelAndView.addObject("Busted", "You Lost");
